...
r.watershed -a -s --overwrite elevation=$M accumulation=$ACCUMSTRING drainage=$DIRSTRING threshold=$THRESH stream=$STREAMSTRING

v.in.ogr input=$NETWORK output=streams
v.in.ogr input=$WATERSHED_POLY output=poly
r.mask vector=poly
v.to.rast input=streams output=stream_raster type=line use=attr attribute_column=COMID
r.to.vect input=stream_raster output=elev_streams type=line
...
# r.stream.basins --overwrite direction=$DIRSTRING stream_rast=stream_raster basins=$BASINSTRING
...
v.in.ascii z=0 input="$MODEL/aux_data/watershed_outlet" output=max_accum_point
v.net -s input=streams threshold=200 points=max_accum_point output=outlet_point operation=connect

v.out.ogr --overwrite -c format=ESRI_Shapefile input=outlet_point type=point output="$MODEL/aux_data/outlet.shp"
v.out.ogr --overwrite -c format=ESRI_Shapefile input=streams type=line output="$MODEL/aux_data/streams.shp"

# # get topologic properties of stream network
# STREAMVECT="stream_vect_$THRESHMETERS"


# r.stream.order --overwrite --quiet accumulation=$ACCUMSTRING elevation=$M stream_rast=stream_raster direction=$DIRSTRING stream_vect=$STREAMVECT
