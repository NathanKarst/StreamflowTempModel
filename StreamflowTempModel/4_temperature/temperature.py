...
class LagrangianSimpleTemperatureChengHeatedGW(Temperature):
    ...
    def __init__(self, rew_id, **kwargs):
        ...
        args = ['upstream_area', 'mannings_n', 'gradient', 'alphaw','eps','rho','cp','kh','sigma','temperature', 'kf', 'tau0', 'ktau', 'Tgw_offset', 'e', 'f', 'g']
        for arg in args: setattr(self, arg, kwargs[arg])        
    ...
    def update(self, dt, **kwargs):
        ...
        hillslope_volumetric_overlandFlow = 1.15741e-11*kwargs['hillslope_volumetric_overlandFlow']
        q = 1.15741e-11*kwargs['volumetric_discharge']
        upstream_area_meters = 0.0001*self.upstream_area
        ...
        # get effective 'thermal' geometry of the channel
        depth = np.exp(-self.e)*upstream_area_meters**self.f*q**self.g
        u = 1/self.mannings_n*depth**(0.667)*self.gradient**0.5
        width = q/(depth*u)
        ...
        Tgw = Tgw - 273.
        Ta_mean = Ta_mean - 273.0
        return (Tgw, Ta_mean, depth, width)
